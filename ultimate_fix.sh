...
let hintEnabled = false, soundEnabled = true;
...
function startBgMusic() {
    ...
    bgMusicInterval = setInterval(() => {
        playTone(notes[i % notes.length], 0.4, 'sine', 0.015);
        i++;
    }, 700);
}
...
function loadProgress() {
    try {
        ...
        if (saved) { level = saved.level||1; wins = saved.wins||0; bonusPoints = saved.bonusPoints||0; }
    } catch(e) {}
}
...
function updateUI() {
    ...
    // به‌روزرسانی نوار سطوح
    $('.level-dot').each(function(index) {
        const dotLevel = index + 1;
        $(this).removeClass('done active locked');
        if (dotLevel < level) $(this).addClass('done');
        else if (dotLevel === level) $(this).addClass('active');
        else if (dotLevel > level && dotLevel <= MAX_LEVEL) $(this).addClass('locked');
    });
}
function advanceLevel() {
    if (level < MAX_LEVEL) {
        ...
        showToast('🏆 شما قهرمان نهایی شدید!');
    }
...
function onDrop(source, target) {
    ...
    moveHistory.push({ move, fenBefore: game.fen() }); // after move
    redoStack = [];
...
async function makeComputerMove() {
    ...
    try {
        ...
        if (data.bestmove) {
            ...
            if (testMove) { game.undo(); moveToApply = { from, to, promotion: promo }; }
        }
...
function updateStatus() {
    ...
    if (game.in_checkmate()) {
        ...
        if (game.turn() !== playerColor) {
            wins++; bonusPoints += 10; saveProgress(); updateUI();
            if (wins >= WINS_TO_ADVANCE) advanceLevel();
        }
        playMateSound();
...
$('#undoBtn').click(() => {
    ...
    // دو حرکت به عقب: حرکت کامپیوتر و حرکت کاربر قبل از آن
    for (let i = 0; i < 2; i++) {
        if (moveHistory.length) { redoStack.push(moveHistory.pop()); game.undo(); }
    }
...
$('#redoBtn').click(() => {
    ...
    for (let i = 0; i < 2; i++) {
        if (redoStack.length) {
            ...
            game.move(entry.move); moveHistory.push(entry);
        }
    }
    board.position(game.fen()); updateStatus();
});
...
$('#flipBtn').click(() => {
    ...
    board.orientation(playerColor); updateStatus();
});
$('#hintBtn').click(function() {
    hintEnabled = !hintEnabled; $(this).toggleClass('active');
    if (!hintEnabled) $('.square-55d63').removeClass('highlight-square');
});
$('#soundToggle').click(function() {
    soundEnabled = !soundEnabled; $(this).toggleClass('active', soundEnabled);
