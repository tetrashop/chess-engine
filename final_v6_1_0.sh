#!/usr/bin/env bash
# final_v6_1_0.sh – انتشار نسخهٔ ۶.۱.۰ بدون حذف تگ قبلی

set -e
cd ~/chess-engine

echo "🔍 ۱. رفتن به main و به‌روزرسانی (نیاز به اینترنت)"
git checkout main
git pull origin main || echo "⚠️ pull ناموفق بود، ادامه می‌دهیم..."

echo "🛠️ ۲. تعمیر فایل‌ها و تنظیمات"
...
# نسخهٔ کاملاً جدید: 6.1.0 (code 600)
sed -i 's/versionCode .*/versionCode 600/' bw-project/build.gradle
sed -i 's/versionName .*/versionName "6.1.0"/' bw-project/build.gradle

echo "⚙️ ۳. ساخت workflow تضمینی"
...
echo "📦 ۴. Commit, Push, و تگ نهایی"
git add -A
git commit -m "Release v6.1.0 – fully stable, no tag deletion"
git push origin main
git tag v6.1.0
git push origin v6.1.0

echo ""
echo "✅ تگ v6.1.0 push شد."
echo "🔗 بعد از ۲ دقیقه به آدرس زیر بروید:"
echo "   https://github.com/tetrashop/chess-engine/releases/tag/v6.1.0"
echo "📱 فایل APK در Assets ظاهر خواهد شد."
