"""
Bitboard definitions, magic bitboard tables, square indexing, and utility functions.
"""
import enum
...
def test_bit(bb: int, sq: int) -> bool:
    return (bb >> sq) & 1 != 0
...
ROOK_DIRS = [8, -8, 1, -1]      # N, S, E, W
BISHOP_DIRS = [9, 7, -7, -9]    # NE, NW, SW, SE

def sliding_attacks(sq: int, occupancy: int, dirs) -> int:
    attacks = 0
    for d in dirs:
        pos = sq
        while True:
            ...
            if pos < 0 or pos > 63:
                break
            # Prevent wrapping around board for horizontal moves
            if (d == 1 or d == -1) and (pos // 8 != sq // 8):
                break
            attacks |= 1 << pos
            if (1 << pos) & occupancy:
                break
...
# Knight and King attack tables precomputed
KNIGHT_ATTACKS = [0]*64
KING_ATTACKS = [0]*64
for sq in range(64):
    bb = 1 << sq
    knight = 0
    knight |= (bb << 17) & ~FILE_A
    knight |= (bb << 10) & ~(FILE_A | FILE_B)
    knight |= (bb >> 6) & ~(FILE_G | FILE_H)
    knight |= (bb >> 15) & ~FILE_H
    knight |= (bb << 15) & ~FILE_H
    knight |= (bb << 6) & ~(FILE_A | FILE_B)
    knight |= (bb >> 10) & ~(FILE_G | FILE_H)
    knight |= (bb >> 17) & ~FILE_A
