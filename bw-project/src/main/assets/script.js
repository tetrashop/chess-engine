...
/* ---------- Replay ---------- */
function startReplay(moves){ /* کد کامل replay */ }
...
async function makeComputerMove(){
    ...
    try {
        ...
        const timeout = setTimeout(() => controller.abort(), 8000); // ۸ ثانیه
        const depth = Math.min(level + 1, 3); // حداکثر عمق ۳
        ...
        // خطا در شبکه یا تایم‌اوت
    }
    // فقط در صورت نبود حرکت معتبر از API
    if (!moveToApply) {
