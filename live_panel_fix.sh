...
function renderGamePanel(){
    ...
    // اگر بازی در جریان باشد (حرکتی انجام شده و بازی تمام نشده)
    if (!game.game_over() && moveHistory.length > 0) {
        ...
        // نمایش سه بازی تمام‌شده با بالاترین امتیاز
        const history = getGameHistory();
...
function onDrop(source, target){
    ...
    updateStatus();
    renderGamePanel();   // بروزرسانی پنل (بازی زنده)
...
async function makeComputerMove(){
    ...
    isThinking = false;
    renderGamePanel();  // به‌روزرسانی پنل
    checkGameEnd();
}
...
function updateStatus(){
    ...
    if(game.in_checkmate()){
        ...
        renderGamePanel();  // حالا بازی تمام شده، پنل بهترین‌ها را نشان می‌دهد
