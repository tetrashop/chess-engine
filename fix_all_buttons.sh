#!/usr/bin/env bash
# fix_all_buttons.sh – ترمیم کامل تمام دکمه‌های فرانت‌اند (بدون باگ)

cd ~/chess-engine
mkdir -p frontend

echo "=== ترمیم تمام دکمه‌ها و عملکردهای فرانت‌اند ==="
...
function renderGamePanel(){
    ...
    top3.forEach(g => {
        ...
        const fenPreview = g.fen ? g.fen.split(' ').slice(0,4).join(' ') : '';
...
$('#newGameBtn').click(()=>{
    ...
    updateStatus(); renderGamePanel(); startBgMusic();
...
$('#undoBtn').click(()=>{
    ...
    board.position(game.fen()); updateStatus(); renderGamePanel();
    if(coachEnabled) fetchBestMoveForCoach();
});
$('#redoBtn').click(()=>{
...
$('#hintBtn').click(function(){
    ...
    $(this).toggleClass('active', hintEnabled);
    if(!hintEnabled) $('.square-55d63').removeClass('highlight-square');
});
$('#coachBtn').click(function(){
    coachEnabled = !coachEnabled;
    $(this).toggleClass('active', coachEnabled);
    if(coachEnabled){
        pendingBestMove=null;
        if(!game.game_over() && game.turn()===userColor) fetchBestMoveForCoach();
...
# ────────────── به‌روزرسانی index.html (بدون تغییر مهم) ──────────────
...
echo ""
echo "✅ تمام دکمه‌ها ترمیم شدند:"
echo "   - بازی جدید / تعویض رنگ / چرخاندن / حرکت‌های مجاز / مربی / صدا"
echo "   - بازگشت (Undo) / پیشروی (Redo)"
echo "   - پنل بازی زنده و replay"
echo ""
echo "🚀 برای انتشار:"
echo "   git add -A && git commit -m 'Fix all buttons: fully functional UI' && git push"
