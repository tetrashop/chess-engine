#!/usr/bin/env bash
# live_replay.sh – نمایش زنده سه بازی برتر، replay خودکار، محاوره‌ای

cd ~/chess-engine
mkdir -p frontend

echo "=== افزودن سیستم replay بازی‌های برتر ==="

# ────────────── 1. index.html (افزودن پنل بازی‌های برتر) ──────────────
...
# ────────────── 2. style.css (افزودن استایل پنل و چت) ──────────────
...
# ────────────── 3. script.js (نسخه کامل با replay و تاریخچه) ──────────────
...
echo ""
echo "✅ سیستم replay بازی‌های برتر با موفقیت اضافه شد."
echo "   - پس از پایان هر بازی، حرکت‌ها و نتیجه ذخیره می‌شود."
echo "   - سه بازی برتر (بالاترین امتیاز) در پنل بالا نمایش داده می‌شوند."
echo "   - با کلیک روی 'replay'، حرکات به‌صورت زنده و با پیام‌های محاوره‌ای پخش می‌شوند."
echo "   - در پایان replay، نتیجه به مدت ۸ ثانیه نشان داده شده و تخته ریست می‌شود."
echo ""
echo "🚀 تست محلی: python backend/app.py"
echo "☁️ انتشار: git add -A && git commit -m 'Add live replay of top 3 games with chat' && git push"
